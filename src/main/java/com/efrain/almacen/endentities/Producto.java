...
@NoArgsConstructor
@Getter
@Builder //Para definir objetos paso a paso
public class Producto {
    ...
    public void descontarCantidad(int cantidad){
        if (cantidad < 0 || cantidad > this.cantidad)
            throw new IllegalArgumentException("La cantidad debe ser positiva y menor oo igual a la cantidad actual");
        this.cantidad -= cantidad;
    }

    public void validarDatos( String nombre, Categoria categoria, BigDecimal precio, Integer cantidad) {
        StringCustomUtils.validarTamanio(nombre, 5, 30, "El nombre es requerido y debe ser entre 5 y 30 caracteres");
        if (categoria == null)
